...
watch(() => props.editingIndex, async (index) => {
    if (index !== null) {
        await nextTick();
...
<template>
    ...
    <div ref="messagesContainer" class="ai-messages" @scroll="$emit('scroll')">
        ...
        <div v-if="messages.length === 0" class="ai-empty-state">
            ...
            <button v-if="!status.isModelLoaded && availableModels.length === 0" class="ai-btn-secondary" @click="$emit('open-models-folder')">
                Open Models Folder
            </button>
            <button v-if="!status.isModelLoaded && availableModels.length > 0" class="ai-btn-secondary" @click="$emit('open-history')">
                Browse History
            </button>
        </div>
            ...
            <div class="ai-message-wrapper">
                ...
                <div v-if="msg.role === 'user' && editingIndex === index" class="ai-message-edit">
                    ...
                    <div class="ai-edit-actions">
                        ...
                        <button class="ai-btn-icon ai-btn-tiny" title="Save & resend" @click="$emit('confirm-edit', index)">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
.ai-empty-icon { color: var(--text3); margin-bottom: 0.75rem; }

.ai-empty-text { font-size: 0.8rem; color: var(--text2); line-height: 1.5; margin: 0; }
...
.ai-btn-secondary {
    ...
    transition: all 0.2s;
    margin-top: 0.5rem;
    &:hover { color: var(--text1); border-color: var(--text2); }
}

.ai-message {
    display: flex;

    &:hover .ai-message-actions { opacity: 1; }

    &.user {
        justify-content: flex-end;
        .ai-message-wrapper { max-width: 85%; }
        .ai-message-content { background: var(--accent-color); color: var(--text1); border-radius: 12px 12px 2px 12px; }
        .ai-message-actions { justify-content: flex-end; }
    }

    &.assistant {
        justify-content: flex-start;
        .ai-message-wrapper { max-width: 90%; }
        .ai-message-content { background: var(--bg-primary); color: var(--text1); border-radius: 12px 12px 12px 2px; }
        .ai-message-actions { justify-content: flex-start; }
    }
...
.ai-message-wrapper { position: relative; }
...
.ai-markdown {
    ...
    :deep(p) { margin: 0 0 0.5em 0; &:last-child { margin-bottom: 0; } }
    :deep(h1), :deep(h2), :deep(h3), :deep(h4) { margin: 0.6em 0 0.3em 0; line-height: 1.3; &:first-child { margin-top: 0; } }
    ...
    :deep(code) { background: rgba(0, 0, 0, 0.15); padding: 0.15em 0.35em; border-radius: 3px; font-size: 0.9em; font-family: 'SF Mono', 'Fira Code', 'Cascadia Code', monospace; }
    :deep(pre) { background: rgba(0, 0, 0, 0.2); padding: 0.6em 0.75em; border-radius: 6px; overflow-x: auto; margin: 0.4em 0; code { background: none; padding: 0; font-size: 0.85em; } }
    :deep(blockquote) { border-left: 3px solid var(--accent-color); margin: 0.4em 0; padding: 0.2em 0.6em; color: var(--text2); }
    :deep(table) { border-collapse: collapse; margin: 0.4em 0; width: 100%; font-size: 0.9em; th, td { border: 1px solid var(--text3); padding: 0.3em 0.5em; text-align: left; } th { background: rgba(0, 0, 0, 0.1); } }
    :deep(hr) { border: none; border-top: 1px solid var(--text3); margin: 0.5em 0; }
    :deep(a) { color: var(--accent-color); text-decoration: underline; }
...
.ai-btn-action {
    ...
    &.ai-btn-action-danger:hover { color: #e55; }
}

.ai-message-edit { display: flex; flex-direction: column; gap: 4px; width: 100%; }
...
.ai-edit-input {
    ...
    resize: vertical;
    outline: none;
    &:focus { border-color: var(--accent-color); box-shadow: 0 0 0 1px var(--accent-color); }
}

.ai-edit-actions { display: flex; justify-content: flex-end; gap: 4px; }

.ai-cursor { animation: blink 0.8s step-end infinite; color: var(--accent-color); font-size: 0.9em; }
...
// Agent edit cards
.ai-agent-edits { display: flex; flex-direction: column; gap: 0.5rem; margin-top: 0.5rem; width: 100%; }
...
.ai-agent-edit-card {
    ...
    &.pending { border-color: var(--accent-color); border-style: dashed; }
    &.approved { border-color: #2ecc71; }
    &.rejected { border-color: #e74c3c; opacity: 0.7; }
...
.ai-agent-edit-file { display: flex; align-items: center; gap: 0.35rem; min-width: 0; flex: 1; }
...
.ai-agent-edit-badge {
    ...
    flex-shrink: 0;
    &.new { background: var(--accent-color); color: var(--base1); }
}
...
.ai-agent-diff-details { border-bottom: 1px solid var(--text3); }
...
.ai-agent-diff { max-height: 300px; overflow-y: auto; }

.ai-agent-diff-section {
    &.removed { background: rgba(231, 76, 60, 0.08); }
    &.added { background: rgba(46, 204, 113, 0.08); }
}
...
.ai-agent-edit-error { padding: 0.4rem 0.65rem; font-size: 0.72rem; color: #e74c3c; border-bottom: 1px solid var(--text3); }

.ai-agent-edit-actions { display: flex; gap: 0.5rem; padding: 0.5rem 0.65rem; }
...
.ai-load-model-btn {
    ...
    transition: opacity 0.15s, transform 0.1s;
    flex-shrink: 0;
    &:hover:not(:disabled) { opacity: 0.85; transform: scale(1.02); }
...
.ai-token-bar-track { flex: 1; height: 3px; background: var(--text3); border-radius: 2px; overflow: hidden; }
...
.ai-token-bar-fill {
    ...
    &.warning { background: #e6a700; }
    &.danger { background: var(--danger-color, #e74c3c); }
}
