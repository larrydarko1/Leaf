...
const CEILINGS = {
    ...
    scss: { limit: 5.5, note: 'presentation — real debt, but it makes things look wrong rather than behave wrong' },
...
if (failures.length > 0) {
    ...
    for (const { what, why, clones } of failures) {
        ...
        for (const c of clones) {
            console.error(`      ${String(c.tokens).padStart(4)} tokens  ${loc(c.firstFile)}  <=>  ${loc(c.secondFile)}`);
        }
        console.error('');
    }
    process.exit(1);
